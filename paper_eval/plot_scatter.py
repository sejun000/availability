...
def main():
    ...
    # 기존: colors = plt.cm.tab10.colors
    # 회색조 팔레트 (밝은 회색 → 검정) ───────▼────────────────────────────
    colors  = ['#aec7e8', '#ffbb78', '#98df8a', '#f7b6d2',
                              '#c5b0d5', '#c7c7c7', '#ffed6f']
    #colors = ['#000000', '#000000', '#525252', '#000000']  # 필요하면 더 추가
    # ───────────────────────────────────────────────────────────────▲────
    ...
    for i, group in enumerate(groups):    
        ...
        if i == 1:  # 세모(두 번째 그룹) → 속 비우기
            handle = plt.Line2D([], [], marker=marker, linestyle='',
                                markersize=8,
                                markerfacecolor='none',          # 면 비움
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            for group in groups:
                ...
                if (zindex == 1):
                    ax.scatter(subset["x"], subset[y_col], facecolors='none', marker=marker_styles[groups.index(group) % len(marker_styles)], s=80, color=color_map[group], alpha=0.8)    
                else:
