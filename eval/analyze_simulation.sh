...
stripe_size=(16 24 48) # m+k
parities=(0 1 2 3 4 5) # --k
#stripe_size=(1)
#parities=(0)
network_stripes=(8 12) # network_m+k
                    ...
                    for c in "${capacity[@]}"; do
                        for network_s in "${network_stripes[@]}"; do
                            for network_p in "${network_parities[@]}"; do
                                ...
                                echo -e "\e[1;32m"
                                echo "Running simulation python3 new_core.py $current_type --output_file $output_file --m $m --k $p --capacity $c $current_type --dwpd $d --tbwpd $tbw  --network_m $network_m --network_k $network_p $additional_params"
                                echo -e "\e[0m"
