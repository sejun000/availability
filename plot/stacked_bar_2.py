...
class StackedBar:
    ...
    def plot_stacked_bar(self, x_col: str, y_cols: list, y_labels: list,
        ...
        ax.set_xlabel(xlabel if xlabel else x_col, fontsize=20, labelpad=10)
        ax.set_ylabel(ylabel if ylabel else "", fontsize=20)
        ax.tick_params(axis='x', which='both', length=0, pad=10, labelsize=20)
        ...
        if show_legend:
            leg = ax.legend(y_labels, loc=legend_location, frameon=True, edgecolor='black', fontsize=20)
        ...
        if title:
            ax.text(0.5, -0.27, title, transform=ax.transAxes,
                    ha='center', fontsize=20, clip_on=False)
            ...
            for patch in ax.patches:
                ...
                if bar_top > y_max:
                    ...
                    ax.text(bar_center_x, y_max + offset, text_str,
                            ha='center', va='bottom', fontsize=14, color='black', clip_on=False)
